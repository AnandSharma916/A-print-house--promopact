import Link from 'next/link';
import LegacyEngine from '@/components/LegacyEngine';
import AboutExtras from '@/components/AboutExtras';
import './about-us.css';
import '../styles/inner-page-fixes.css';

export const metadata = {
  title: "About Us | A India Print House — 76+ Years of Playing Card Craftsmanship",
  description: "The story of A India Print House — a 4th-generation, 76+ year old playing card manufacturer in New Delhi. Our heritage, mission, vision, craft, and the people behind every bespoke deck.",
  alternates: { canonical: "/about-us" },
};

export default function AboutUsPage() {
  return (
    <div className="page-root">
      <LegacyEngine variant="product" />
      <AboutExtras />
      {/* NAVBAR (matched to index.html) */}
      <header id={"site-header"}>
        <div className={"hdr-wrap"}>
          <Link href={"/"} className={"hdr-logo"}>
            <img src={"/img/aiph-logo.avif"} alt={"AIPH"} className={"hdr-logo-img"} />
            <div className={"hdr-logo-text"}>
              <span className={"hlt-main"}>
                A India
              </span>
              <span className={"hlt-sub"}>
                Print House
              </span>
            </div>
          </Link>
          <nav className={"hdr-nav"}>
            <Link href={"/"} className={"hdr-link"}>
              Home
            </Link>
            <div className={"hdr-dropdown"}>
              <a href={"#"} className={"hdr-link"}>
                Collection
                <span className={"hdr-caret"}>
                  ▾
                </span>
              </a>
              <div className={"hdr-menu"}>
                <Link href={"/premium-playing-cards"} className={"hdr-menu-link"}>
                  Premium Playing Cards
                </Link>
                <Link href={"/promotional-playing-cards"} className={"hdr-menu-link"}>
                  Promotional Playing Cards
                </Link>
                <Link href={"/advertisement-playing-cards"} className={"hdr-menu-link"}>
                  Advertisement Playing Cards
                </Link>
                <Link href={"/card-games"} className={"hdr-menu-link"}>
                  Card Games
                </Link>
                <Link href={"/corporate-playing-cards"} className={"hdr-menu-link"}>
                  Corporate Playing Cards
                </Link>
                <Link href={"/souvenir-playing-cards"} className={"hdr-menu-link"}>
                  Customised Playing Cards
                </Link>
                <Link href={"/branded-playing-cards"} className={"hdr-menu-link"}>
                  Branded Playing Cards
                </Link>
                <Link href={"/poker-cards"} className={"hdr-menu-link"}>
                  Poker Cards
                </Link>
                <Link href={"/educational-cards"} className={"hdr-menu-link"}>
                  Educational Cards
                </Link>
                <Link href={"/flash-cards"} className={"hdr-menu-link"}>
                  Flash Cards
                </Link>
              </div>
            </div>
            <Link href={"/about-us"} className={"hdr-link"} style={{ color: "var(--gold)" }}>
              About Us
            </Link>
            <Link href={"/contact-us"} className={"hdr-link"}>
              Contact us
            </Link>
          </nav>
          <a href={"#"} className={"hdr-btn magnetic"}>
            <span>
              Bespoke Order
            </span>
            <div className={"hdr-btn-glow"}></div>
          </a>
          <button className={"hdr-burger"} id={"hdr-burger"} aria-label={"Open menu"}>
            <div className={"burger-bar b1"}></div>
            <div className={"burger-bar b2"}></div>
          </button>
        </div>
      </header>
      {/* FULLSCREEN NAV */}
      <div className={"nav-overlay"} id={"nav-overlay"}>
        <div className={"no-bg-panel"}></div>
        <div className={"no-content"}>
          <div className={"no-links"}>
            <Link href={"/"} className={"no-link"}>
              <em className={"no-idx"}>
                01
              </em>
              <span>
                Home
              </span>
            </Link>
            <div className={"no-link no-collection-group"}>
              <div className={"no-collection-head"}>
                <em className={"no-idx"}>
                  02
                </em>
                <span>
                  Collection
                </span>
              </div>
              <div className={"no-collection-links"}>
                <Link href={"/premium-playing-cards"} className={"no-col-link"}>
                  Premium Playing Cards
                </Link>
                <Link href={"/promotional-playing-cards"} className={"no-col-link"}>
                  Promotional Playing Cards
                </Link>
                <Link href={"/advertisement-playing-cards"} className={"no-col-link"}>
                  Advertisement Playing Cards
                </Link>
                <Link href={"/card-games"} className={"no-col-link"}>
                  Card Games
                </Link>
                <Link href={"/corporate-playing-cards"} className={"no-col-link"}>
                  Corporate Playing Cards
                </Link>
                <Link href={"/souvenir-playing-cards"} className={"no-col-link"}>
                  Customised Playing Cards
                </Link>
                <Link href={"/branded-playing-cards"} className={"no-col-link"}>
                  Branded Playing Cards
                </Link>
                <Link href={"/poker-cards"} className={"no-col-link"}>
                  Poker Cards
                </Link>
                <Link href={"/educational-cards"} className={"no-col-link"}>
                  Educational Cards
                </Link>
                <Link href={"/flash-cards"} className={"no-col-link"}>
                  Flash Cards
                </Link>
              </div>
            </div>
            <Link href={"/about-us"} className={"no-link"}>
              <em className={"no-idx"}>
                03
              </em>
              <span>
                About Us
              </span>
            </Link>
            <a href={"#about-why"} className={"no-link"}>
              <em className={"no-idx"}>
                04
              </em>
              <span>
                Why Us
              </span>
            </a>
            <Link href={"/contact-us"} className={"no-link"}>
              <em className={"no-idx"}>
                05
              </em>
              <span>
                Contact Us
              </span>
            </Link>
          </div>
          <div className={"no-side"}>
            <div className={"no-suits-grid"}>
              <span>
                ♠
              </span>
              <span className={"r"}>
                ♥
              </span>
              <span>
                ♣
              </span>
              <span className={"r"}>
                ♦
              </span>
            </div>
            <div className={"no-contact-info"}>
              <p>
                marketingaiph7@gmail.com
              </p>
              <p>
                +91-9810614016
              </p>
              <p>
                New Delhi, India
              </p>
            </div>
          </div>
        </div>
        <button className={"no-close"} id={"no-close"}>
          ✕
        </button>
      </div>
      {/* Hero Section */}
      <section className={"hero"}>
        <div className={"hero__bg"}>
          <img alt={"A India Print House — About Us"} src={"/img/slider-bg-2.avif"} />
          <div className={"hero__overlay"}></div>
          <div className={"hero__gradient"}></div>
        </div>
        <div id={"hero-particles"}></div>
        <div className={"hero__content"}>
          <span className={"hero__eyebrow"}>
            OUR STORY
          </span>
          <h1 className={"hero__title"}>
            Crafting Stories,
            <br />
            <em>
              One Card at a Time
            </em>
          </h1>
          <p className={"hero__text"}>
            For over seven decades, A India Print House has turned ink, paper, and imagination into playing cards that brands, families, and collectors treasure. This is who we are.
          </p>
          <div>
            <a href={"#about-team"}>
              <button className={"btn-cream magnetic"}>
                Meet The Makers
              </button>
            </a>
          </div>
        </div>
        {/* Scroll Indicator */}
        <div className={"hero__scroll"}>
          <span>
            Scroll
          </span>
          <div className={"hero__scroll-line"}></div>
        </div>
      </section>
      {/* Introduction Section (Heritage / Who We Are) */}
      <section className={"section section--padded bg-surface"}>
        <div className={"container heritage__grid"}>
          <div className={"heritage__text"}>
            <span className={"eyebrow"}>
              Who We Are
            </span>
            <h2 className={"headline-lg heritage__title"} data-split="">
              A Family Press, Four Generations Strong
            </h2>
            <p className={"body-md heritage__p"}>
              A India Print House is a trusted manufacturer and supplier of premium playing cards, promotional decks, corporate cards, educational and flash cards, and bespoke card games. From our home in New Delhi, we serve businesses, casinos, and brands across India and overseas.
            </p>
            <p className={"body-md heritage__p heritage__p--last"}>
              What began as a humble family press has grown into a fourth-generation studio that marries old-world tactile craft with modern offset precision — so every deck that leaves our house is a small masterpiece.
            </p>
            <div className={"discover"}>
              <div className={"discover__icon"}>
                <span className={"material-symbols-outlined"}>
                  arrow_forward
                </span>
              </div>
              <span className={"label-md discover__label"}>
                Read our journey
              </span>
            </div>
          </div>
          <div className={"heritage__media"}>
            <figure className={"vintage-photo"}>
              <div className={"vintage-photo__frame no-parallax"}>
                <img alt={"India House Playing Card Manufacturers, Sadar Bazar, Delhi — Est. 1951"} src={"/img/Our-History.avif"} />
                <div className={"vintage-photo__grain"}></div>
              </div>
              <figcaption className={"vintage-photo__cap"}>
                <span className={"vpc-suits"}>
                  ♠ ♥ ♣ ♦
                </span>
                India House · Sadar Bazar, Delhi —
                <em>
                  Est. 1951
                </em>
              </figcaption>
            </figure>
          </div>
        </div>
        {/* Animated stats (home-page style counters) */}
        <div className={"container about-stats"}>
          <div className={"about-stat"}>
            <div className={"about-stat__num"}>
              <span data-count={"76"}>
                0
              </span>
              <span className={"about-stat__unit"}>
                +
              </span>
            </div>
            <div className={"about-stat__label"}>
              Years of
              <br />
              Craftsmanship
            </div>
          </div>
          <div className={"about-stat"}>
            <div className={"about-stat__num"}>
              <span data-count={"1000"}>
                0
              </span>
              <span className={"about-stat__unit"}>
                +
              </span>
            </div>
            <div className={"about-stat__label"}>
              Bespoke
              <br />
              Projects
            </div>
          </div>
          <div className={"about-stat"}>
            <div className={"about-stat__num"}>
              <span data-count={"100"}>
                0
              </span>
              <span className={"about-stat__unit"}>
                %
              </span>
            </div>
            <div className={"about-stat__label"}>
              Client
              <br />
              Satisfaction
            </div>
          </div>
          <div className={"about-stat"}>
            <div className={"about-stat__num"}>
              <span data-count={"4"}>
                0
              </span>
              <span className={"about-stat__unit"}>
                th
              </span>
            </div>
            <div className={"about-stat__label"}>
              Generation
              <br />
              Family Press
            </div>
          </div>
        </div>
        <p className={"about-stats-note"}>
          Seven decades of experience, built on
          <em>
            trust&nbsp;and&nbsp;quality.
          </em>
        </p>
      </section>
      {/* The Craft Section (Atropos 3D tilt cards) */}
      <section className={"section bg-dark"}>
        <div className={"container section--padded"}>
          <div className={"section-head"}>
            <h2 className={"headline-lg"} data-split="">
              The Hands Behind Every Deck
            </h2>
            <div className={"section-head__rule"} data-scroll="" data-scroll-class={"is-inview"}></div>
            <p className={"body-lg section-head__text"}>
              Skilled craftspeople, premium linen-finish stocks, and advanced UV printing — every card is checked, coated, and cut with obsessive care before it ever reaches your table.
            </p>
          </div>
          <div className={"durable__grid"}>
            <div className={"atropos deck-atropos"}>
              <div className={"atropos-scale"}>
                <div className={"atropos-rotate"}>
                  <div className={"atropos-inner"}>
                    <div className={"deck-card"}>
                      <div className={"deck-frame"} aria-hidden={"true"}></div>
                      <img alt={"Craftsmanship — premium decks"} src={"/img/playing-1.avif"} />
                      <div className={"deck-card__caption"}>
                        <div>
                          <span className={"label-sm deck-card__kicker"}>
                            Built to Last
                          </span>
                          <h3 className={"headline-md deck-card__title"}>
                            Smooth Handling &amp; Snap
                          </h3>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className={"atropos deck-atropos"}>
              <div className={"atropos-scale"}>
                <div className={"atropos-rotate"}>
                  <div className={"atropos-inner"}>
                    <div className={"deck-card"}>
                      <div className={"deck-frame"} aria-hidden={"true"}></div>
                      <img alt={"Drink Up — custom card game packaging"} src={"/img/drink-up.avif"} className={"no-parallax"} style={{ padding: "12%" }} />
                      <div className={"deck-card__caption"}>
                        <div>
                          <span className={"label-sm deck-card__kicker"}>
                            Designed In-House
                          </span>
                          <h3 className={"headline-md deck-card__title"}>
                            Custom Art &amp; Packaging
                          </h3>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
      {/* 3D Card Showcase (Three.js) */}
      <section className={"card-showcase"} id={"card-showcase"}>
        <div className={"card-showcase__content container section--padded"}>
          <span className={"eyebrow"}>
            In Every Suit
          </span>
          <h2 className={"display-lg card-showcase__title"}>
            Heritage Built Into Our DNA
          </h2>
          <p className={"card-showcase__text"}>
            Spades, hearts, clubs, diamonds — four suits, four generations. The same family hands that printed our first deck still guide every press that runs today.
          </p>
        </div>
      </section>
      {/* What We Stand For (Bento Grid: Mission / Vision / Values) */}
      <section className={"section section--padded bg-surface"}>
        <div className={"container"}>
          <div className={"specs__head"}>
            <span className={"eyebrow specs__kicker"}>
              What We Stand For
            </span>
            <h2 className={"headline-lg specs__title"} data-split="">
              Our Mission, Vision &amp; Values
            </h2>
          </div>
          <div className={"bento"}>
            {/* Large Feature: Mission */}
            <div className={"bento__cell bento__cell--feature"}>
              <div>
                <span className={"material-symbols-outlined bento__icon bento__icon--lg"}>
                  flag
                </span>
                <h4 className={"headline-md bento__feature-title"}>
                  Our Mission
                </h4>
                <p className={"body-md bento__desc"}>
                  To deliver innovative, high-quality printing solutions that help businesses, educators, and organisations achieve their goals — products that are visually striking, durable, and genuinely value-driven.
                </p>
              </div>
              <div className={"bento__meta"}>
                <span className={"label-sm bento__meta-label"}>
                  Quality is our standard
                </span>
                <span className={"material-symbols-outlined"} style={{ color: "var(--foil-gold)" }}>
                  check_circle
                </span>
              </div>
            </div>
            {/* Small Features: Values */}
            <div className={"bento__cell"}>
              <span className={"material-symbols-outlined bento__icon"}>
                visibility
              </span>
              <h5 className={"label-md bento__heading"}>
                Our Vision
              </h5>
              <p className={"bento__desc bento__desc--sm"}>
                To be India's most trusted maker of customised card products at home and overseas.
              </p>
            </div>
            <div className={"bento__cell"}>
              <span className={"material-symbols-outlined bento__icon"}>
                workspace_premium
              </span>
              <h5 className={"label-md bento__heading"}>
                Uncompromising Quality
              </h5>
              <p className={"bento__desc bento__desc--sm"}>
                Premium stocks, protective coatings, and a finish that exceeds expectations.
              </p>
            </div>
            <div className={"bento__cell"}>
              <span className={"material-symbols-outlined bento__icon"}>
                handshake
              </span>
              <h5 className={"label-md bento__heading"}>
                Customer-First
              </h5>
              <p className={"bento__desc bento__desc--sm"}>
                We work closely with every client until the final product matches their vision.
              </p>
            </div>
            <div className={"bento__cell"}>
              <span className={"material-symbols-outlined bento__icon"}>
                lightbulb
              </span>
              <h5 className={"label-md bento__heading"}>
                Innovation
              </h5>
              <p className={"bento__desc bento__desc--sm"}>
                Continuous improvement in design, materials, and printing technology.
              </p>
            </div>
          </div>
        </div>
      </section>
      {/* Our Journey (timeline, re-skin of apps list) */}
      <section className={"section section--padded bg-dark"}>
        <div className={"container apps__grid"}>
          <div>
            <h2 className={"headline-lg apps__title"}>
              Our Journey
            </h2>
            <ul className={"apps__list"}>
              <li className={"apps__item journey__item"}>
                <span className={"headline-md apps__num journey__year"}>
                  1951
                </span>
                <span className={"headline-md apps__name"}>
                  A family press is founded in New Delhi
                </span>
              </li>
              <li className={"apps__item journey__item"}>
                <span className={"headline-md apps__num journey__year"}>
                  1980s
                </span>
                <span className={"headline-md apps__name"}>
                  Specialising in playing &amp; educational cards
                </span>
              </li>
              <li className={"apps__item journey__item"}>
                <span className={"headline-md apps__num journey__year"}>
                  2000s
                </span>
                <span className={"headline-md apps__name"}>
                  Modern offset &amp; UV printing adopted
                </span>
              </li>
              <li className={"apps__item journey__item"}>
                <span className={"headline-md apps__num journey__year"}>
                  2010s
                </span>
                <span className={"headline-md apps__name"}>
                  Branded decks for national &amp; global brands
                </span>
              </li>
              <li className={"apps__item journey__item"}>
                <span className={"headline-md apps__num journey__year"}>
                  Today
                </span>
                <span className={"headline-md apps__name"}>
                  4th generation, 1000+ bespoke projects
                </span>
              </li>
            </ul>
          </div>
          <div className={"apps__media"}>
            <img alt={"A India Print House journey"} src={"/img/playing-2-centred.avif"} />
            <div className={"apps__media-ring"}></div>
          </div>
        </div>
      </section>
      {/* The People Behind the Press (Swiper carousel — home-page library) */}
      <section className={"section section--padded bg-cream"} id={"about-team"}>
        <div className={"team__head"}>
          <span className={"eyebrow"}>
            The People Behind the Press
          </span>
          <h2 className={"headline-lg"} data-split="">
            Hands, Hearts &amp; Suits
          </h2>
        </div>
        <div className={"swiper about-swiper"}>
          <div className={"swiper-wrapper"}>
            <div className={"swiper-slide"}>
              <div className={"team-card"}>
                <div className={"team-card__avatar"}>
                  ♠
                </div>
                <span className={"team-card__suit"}>
                  ♠
                </span>
                <h4>
                  The Founding Generation
                </h4>
                <span className={"team-card__role"}>
                  Heritage &amp; Legacy
                </span>
                <p>
                  The family that started it all — setting the standard of quality and care we still hold to every single day.
                </p>
              </div>
            </div>
            <div className={"swiper-slide"}>
              <div className={"team-card"}>
                <div className={"team-card__avatar"}>
                  ♥
                </div>
                <span className={"team-card__suit"}>
                  ♥
                </span>
                <h4>
                  Design Studio
                </h4>
                <span className={"team-card__role"}>
                  Art &amp; Customisation
                </span>
                <p>
                  Our designers turn brand briefs, logos, and ideas into finished card faces, backs, and packaging.
                </p>
              </div>
            </div>
            <div className={"swiper-slide"}>
              <div className={"team-card"}>
                <div className={"team-card__avatar"}>
                  ♣
                </div>
                <span className={"team-card__suit"}>
                  ♣
                </span>
                <h4>
                  Press &amp; Production
                </h4>
                <span className={"team-card__role"}>
                  Printing &amp; Finishing
                </span>
                <p>
                  The hands at the machines — calibrating colour, coating, cutting, and inspecting every run.
                </p>
              </div>
            </div>
            <div className={"swiper-slide"}>
              <div className={"team-card"}>
                <div className={"team-card__avatar"}>
                  ♦
                </div>
                <span className={"team-card__suit"}>
                  ♦
                </span>
                <h4>
                  Client Partners
                </h4>
                <span className={"team-card__role"}>
                  Service &amp; Delivery
                </span>
                <p>
                  For over four decades, many of our clients have continued to place their trust in us, proof that quality and consistency build lasting partnerships.
                </p>
              </div>
            </div>
          </div>
          <div className={"swiper-pagination about-pager"}></div>
        </div>
      </section>
      {/* Promise / Signature (Lottie accent + Rough Notation box) */}
      <section className={"section section--padded bg-surface"}>
        <div className={"container custom"}>
          <h2 className={"headline-lg custom__title"} data-split="">
            More Than a Printer
          </h2>
          <p className={"body-lg custom__quote"}>
            "We don't just print cards — we create products that help brands connect, promote, and grow. From the first idea to the finished deck, every order carries our family's name and our promise of quality."
          </p>
          <div className={"custom__sig"}>
            <div className={"custom__sig-rule"}></div>
            <span className={"label-md custom__sig-label"}>
              A India Print House
            </span>
          </div>
        </div>
      </section>
      {/* Our Promise / Why Us & Benefits Section */}
      <section className={"section section--padded bg-cream"} id={"about-why"}>
        <div className={"container why__grid"}>
          <div className={"why__quote"}>
            <blockquote className={"headline-lg"}>
              "Quality is not just our promise; it is our standard."
            </blockquote>
          </div>
          <div className={"why__benefits"}>
            <div className={"why__benefits-grid"}>
              <div>
                <h6 className={"label-md benefit__title"} data-scroll="" data-scroll-class={"is-inview"}>
                  Premium Quality
                </h6>
                <p className={"benefit__desc"}>
                  Advanced printing and expert craftsmanship that meets the highest international standards.
                </p>
              </div>
              <div>
                <h6 className={"label-md benefit__title"} data-scroll="" data-scroll-class={"is-inview"}>
                  Fully Customised
                </h6>
                <p className={"benefit__desc"}>
                  Bespoke designs, sizes, and packaging tailored to your brand and purpose.
                </p>
              </div>
              <div>
                <h6 className={"label-md benefit__title"} data-scroll="" data-scroll-class={"is-inview"}>
                  Honest Pricing
                </h6>
                <p className={"benefit__desc"}>
                  High-value, cost-effective printing optimised for every budget and bulk order.
                </p>
              </div>
              <div>
                <h6 className={"label-md benefit__title"} data-scroll="" data-scroll-class={"is-inview"}>
                  On-Time Delivery
                </h6>
                <p className={"benefit__desc"}>
                  A streamlined, fast production workflow that respects your deadlines.
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>
      {/* Final CTA Section (Granim gradient + confetti) */}
      <section className={"section bg-dark cta"}>
        <canvas id={"cta-granim"}></canvas>
        <div className={"container section--padded"}>
          <h2 className={"display-lg cta__title"}>
            Let's Build Something Memorable
          </h2>
          <div className={"cta__contacts"}>
            <div className={"cta__contact"}>
              <span className={"material-symbols-outlined"}>
                mail
              </span>
              <a className={"headline-md"} href={"mailto:marketingaiph7@gmail.com"}>
                marketingaiph7@gmail.com
              </a>
            </div>
            <div className={"cta__contact"}>
              <span className={"material-symbols-outlined"}>
                call
              </span>
              <a className={"headline-md"} href={"tel:+919810614016"}>
                +91 9810614016
              </a>
            </div>
          </div>
          <button className={"btn-gold magnetic"}>
            Start Your Project
          </button>
        </div>
      </section>
      {/* FOOTER (matched to index.html) */}
      <footer id={"site-footer"}>
        <div className={"footer-suits-mq"}>
          <div className={"fsm-track"}>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
            <span>
              ♠
            </span>
            <span className={"r"}>
              ♥
            </span>
            <span>
              ♣
            </span>
            <span className={"r"}>
              ♦
            </span>
          </div>
        </div>
        <div className={"footer-big-name"}>
          <div className={"fbn-track"}>
            <span className={"fbn-inner"}>
              A India Print House &nbsp;·&nbsp; A India Print House &nbsp;·&nbsp; A India Print House &nbsp;·&nbsp;
            </span>
            <span className={"fbn-inner"}>
              A India Print House &nbsp;·&nbsp; A India Print House &nbsp;·&nbsp; A India Print House &nbsp;·&nbsp;
            </span>
          </div>
        </div>
        <div className={"footer-body container"}>
          <div className={"footer-top"}>
            <div className={"ft-brand-col"}>
              <div className={"ft-logo"}>
                <img src={"/img/aiph-logo.avif"} alt={"AIPH"} className={"ft-logo-img"} />
                AIPH
              </div>
              <p>
                India's premium playing card manufacturer. Crafting excellence, one card at a time.
              </p>
            </div>
            <div className={"ft-links-cols"}>
              <div className={"ftl-col"}>
                <h4>
                  Products
                </h4>
                <ul>
                  <li>
                    <Link href={"/premium-playing-cards"}>
                      Premium Playing Cards
                    </Link>
                  </li>
                  <li>
                    <Link href={"/promotional-playing-cards"}>
                      Promotional Cards
                    </Link>
                  </li>
                  <li>
                    <Link href={"/corporate-playing-cards"}>
                      Corporate Playing Cards
                    </Link>
                  </li>
                  <li>
                    <Link href={"/card-games"}>
                      Card Games
                    </Link>
                  </li>
                </ul>
              </div>
              <div className={"ftl-col"}>
                <h4>
                  Company
                </h4>
                <ul>
                  <li>
                    <Link href={"/about-us"}>
                      About Us
                    </Link>
                  </li>
                  <li>
                    <a href={"#about-why"}>
                      Why Us
                    </a>
                  </li>
                  <li>
                    <Link href={"/#testimonials"}>
                      Testimonials
                    </Link>
                  </li>
                  <li>
                    <Link href={"/#contact"}>
                      Contact us
                    </Link>
                  </li>
                </ul>
              </div>
              <div className={"ftl-col"}>
                <h4>
                  Reach Us
                </h4>
                <ul>
                  <li>
                    <a href={"mailto:marketingaiph7@gmail.com"}>
                      marketingaiph7@gmail.com
                    </a>
                  </li>
                  <li>
                    <a href={"tel:+91-9810614016"}>
                      +91-9810614016
                    </a>
                  </li>
                  <li>
                    <a href={"tel:+91-9711145467"}>
                      +91-9711145467
                    </a>
                  </li>
                  <li>
                    <span>
                      A-10/2 Mayapuri Industrial Area,
                      <br />
                      Phase 1, New Delhi 110064
                    </span>
                  </li>
                </ul>
              </div>
            </div>
          </div>
          <div className={"footer-bottom"}>
            <p>
              Designed And Managed By Promopact Marketing India Pvt. Ltd. All Rights Reserved.
            </p>
            <div className={"fb-suits"}>
              ♠ ♥ ♦ ♣
            </div>
          </div>
        </div>
      </footer>
      {/* Navbar behavior + all animations are handled by assets/animations.js */}
      {/* FLOATING SOCIAL ICONS (matched to index.html) */}
      <div className={"floating-social-group"}>
        <a href={"https://wa.me/919810614016"} target={"_blank"} rel={"noopener noreferrer"} className={"fsi-btn fsi-whatsapp"} aria-label={"WhatsApp"}>
          <svg viewBox={"0 0 24 24"} fill={"currentColor"}>
            <path d={"M12.012 2c-5.506 0-9.989 4.478-9.99 9.984a9.96 9.96 0 001.37 5.054L2 22l5.077-1.33a9.929 9.929 0 004.93 1.302c5.506 0 9.99-4.478 9.99-9.984C22.007 6.478 17.519 2 12.012 2zm6.09 14.122c-.25.703-1.455 1.285-1.996 1.353-.497.062-1.15.115-3.328-.785-2.784-1.15-4.57-3.98-4.71-4.167-.139-.187-1.135-1.507-1.135-2.877 0-1.37.712-2.042.966-2.316.254-.275.556-.343.74-.343.185 0 .37.004.532.012.169.008.397-.064.62.469.23.55.787 1.916.855 2.053.067.137.112.298.02.482-.09.183-.135.297-.27.457-.134.159-.283.356-.404.477-.135.136-.277.284-.12.553.157.27.7 1.151 1.502 1.866.802.714 1.477.935 1.684 1.026.208.09.33.076.452-.064.122-.14.523-.609.664-.817.14-.207.28-.175.472-.104.193.07 1.22.576 1.43.682.208.106.347.16.398.248.05.088.05.511-.2 1.122z"}></path>
          </svg>
          <span className={"fsi-tooltip"}>
            WhatsApp
          </span>
        </a>
        <a href={"tel:+919810614016"} className={"fsi-btn fsi-call"} aria-label={"Call Us"}>
          <svg viewBox={"0 0 24 24"} fill={"currentColor"}>
            <path d={"M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"}></path>
          </svg>
          <span className={"fsi-tooltip"}>
            Call Us
          </span>
        </a>
        <a href={"mailto:marketingaiph7@gmail.com"} className={"fsi-btn fsi-email"} aria-label={"Email Us"}>
          <svg viewBox={"0 0 24 24"} fill={"currentColor"}>
            <path d={"M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"}></path>
          </svg>
          <span className={"fsi-tooltip"}>
            Email Us
          </span>
        </a>
        <a href={"https://instagram.com"} target={"_blank"} rel={"noopener noreferrer"} className={"fsi-btn fsi-instagram"} aria-label={"Instagram"}>
          <svg viewBox={"0 0 24 24"} fill={"currentColor"}>
            <path d={"M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.051.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"}></path>
          </svg>
          <span className={"fsi-tooltip"}>
            Instagram
          </span>
        </a>
        <a href={"https://facebook.com"} target={"_blank"} rel={"noopener noreferrer"} className={"fsi-btn fsi-facebook"} aria-label={"Facebook"}>
          <svg viewBox={"0 0 24 24"} fill={"currentColor"}>
            <path d={"M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c4.56-.93 8-4.96 8-9.75z"}></path>
          </svg>
          <span className={"fsi-tooltip"}>
            Facebook
          </span>
        </a>
      </div>
      {/* All shared animations run from the shared module */}
      {/* About-page extras: Swiper carousel + animated stat counters (Swiper, GSAP CustomEase & ScrollTrigger — the home-page stack) */}
    </div>
  );
}
