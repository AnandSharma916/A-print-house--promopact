/* ============================================================
   ...
   Ported from the standalone <script src="assets/animations.js"> IIFE to a
   module the React tree drives: it now runs on mount instead of on
   DOMContentLoaded and returns a teardown, so client-side navigation between
   product pages does not stack up Lenis/Locomotive instances or leave dead
   ScrollTriggers pinning sections that no longer exist.
   ============================================================ */
export default function initLegacyAnimations() {
  'use strict';
  ...
  /* addEventListener that unregisters itself on teardown. */
  function on(target, type, fn, opts) {
    target.addEventListener(type, fn, opts);
    disposers.push(function () { target.removeEventListener(type, fn, opts); });
  }

  /* requestAnimationFrame loop that stops on teardown. */
  function raf(step) {
    var id = 0;
    var stopped = false;
    (function frame(t) {
    ...
    disposers.push(function () { stopped = true; cancelAnimationFrame(id); });
  }
  ...
  function initGSAP() {
    ...
    /* We mount after 'load' has already fired, so refresh on a short delay
       instead — images decoding late used to be what this waited for. */
    var refreshTimer = setTimeout(function () { ScrollTrigger.refresh(); }, 150);
    disposers.push(function () { clearTimeout(refreshTimer); });
  }
  ...
  function initLenis() {
    ...
    if (window.gsap && window.ScrollTrigger) {
      ...
      disposers.push(function () { gsap.ticker.remove(tick); });
    } else {
      (function raf(t) { lenis.raf(t); rafId = requestAnimationFrame(raf); })();
      disposers.push(function () { cancelAnimationFrame(rafId); });
    }
    window.__lenis = lenis;
    disposers.push(function () { lenis.destroy(); window.__lenis = null; });
  }
  ...
  function initThree() {
    ...
    if ('IntersectionObserver' in window) {
      var io = new IntersectionObserver(function (en) { hostVisible = en[0].isIntersecting; });
      io.observe(host);
      disposers.push(function () { io.disconnect(); });
    }
    ...
    /* The canvas is appended to #card-showcase by hand, so remove it by hand. */
  ...
  function initLocomotive() {
    ...
    if (typeof LocomotiveScroll !== 'undefined') {
      ...
      disposers.push(function () {
        try { if (window.__loco) window.__loco.destroy(); } catch (e) { /* already torn down */ }
        window.__loco = null;
  ...
  function initAtroposGranim() {
    ...
    if (!REDUCED && typeof window.Granim === 'function' && document.getElementById('cta-granim')) {
      try {
        ...
        disposers.push(function () {
          try { granim.destroy(); } catch (e) { /* already destroyed */ }
        });
      } catch (e) { /* CTA keeps its solid dark background */ }
    }
  }

  /* Mounted by <LegacyAnimations>, so the document is already parsed and the
     old DOMContentLoaded listener would never have fired. */
  [initNav, initGSAP, initLenis, initThree, initAnime,
   initLocomotive, initParticles, initText, initAtroposGranim]
    .forEach(function (fn) {
      try { fn(); } catch (e) { console.error('legacy-animations: ' + fn.name + ' failed:', e); }
    });

  return function teardown() {
    disposers.forEach(function (fn) {
      try { fn(); } catch (e) { console.error('legacy-animations teardown step failed:', e); }
    });
    disposers.length = 0;
    ...
    try {
      if (window.tsParticles) window.tsParticles.dom().forEach(function (c) { c.destroy(); });
    } catch (e) { /* tsParticles never loaded */ }
  };
}
